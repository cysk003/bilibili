...
# Auto download cloud image from Internet, used as base, size as base image size
...
# ==== some lines to undefine a domain ====
# [ "$(virsh -q domstate "$VM")" = "running" ] && virsh -q destroy "$VM" && echo "Info: $VM forced shutdown."
# [ -z "$(virsh -q domblklist "$VM" | awk '$NF != "-" {print $NF}')" ] && echo "Error: no block device for $VM" && exit 1
# virsh -q undefine --remove-all-storage "$VM" && echo "Successfully undefined $VM"
# ==== End of lines to undefine a domain ====
...
cloud() {
  ...
  if [ -n "$BASE" ]; then
    ...
    if [ ! -r "$BASE" ]; then
      ...
      if ! axel -n 10 "$URL" -o "$BASE_DIR"; then
...
if [ "$(basename "$IMG" .iso).iso" = "$(basename "$IMG")" ]; then
  ...
  if [ -n "$DISK" ]; then
    ...
    echo "Info:Check with cloud image ..."
    cloud
  fi
fi
